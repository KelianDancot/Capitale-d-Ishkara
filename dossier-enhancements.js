(() => {
  ...
  renderDossier = function (tribeKey, clanKey) {
    ...
    requestAnimationFrame(() => decorateClassRecommendations(tribeKey, clanKey));
  };
  ...
  try {
    ...
    if (progress?.tribe && progress?.clan && tribes[progress.tribe]?.clans?.[progress.clan]) {
      requestAnimationFrame(() => decorateClassRecommendations(progress.tribe, progress.clan));
    }
  } catch (_) {}
})();